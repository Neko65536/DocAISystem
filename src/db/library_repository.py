...
_MEMORY_SPACES: Dict[str, LibrarySpaceRow] = {}
_MEMORY_DOCS: Dict[str, LibraryDocRow] = {}
...
def _memory_doc_count(space_id: str, user_id: Optional[str] = None) -> int:
    return sum(
        1
        for doc in _MEMORY_DOCS.values()
...
def _memory_space_visible(space: LibrarySpaceRow, user_id: Optional[str] = None) -> bool:
    return not user_id or space.user_id == user_id


def _memory_doc_visible(doc: LibraryDocRow, user_id: Optional[str] = None) -> bool:
    return doc.deleted_at is None and (not user_id or doc.user_id == user_id)
...
def create_library_space(
    ...
    """创建文档空间"""
    cfg = config or get_config()
    if not _db_enabled(cfg):
        now = _utc_now()
        space_id = str(uuid.uuid4())
        space = LibrarySpaceRow(
            id=space_id,
            user_id=user_id,
            name=name,
            icon=icon,
...
def get_library_space_by_id(
    ...
    cfg = config or get_config()
    if not _db_enabled(cfg):
        space = _MEMORY_SPACES.get(space_id)
        if not space or not _memory_space_visible(space, user_id):
            return None
        space.doc_count = _memory_doc_count(space_id, user_id)
        return space
...
def get_library_spaces(
    ...
    cfg = config or get_config()
    if not _db_enabled(cfg):
        spaces = [space for space in _MEMORY_SPACES.values() if _memory_space_visible(space, user_id)]
        for space in spaces:
            space.doc_count = _memory_doc_count(space.id, user_id)
        return sorted(spaces, key=lambda item: item.created_at, reverse=True)
...
def delete_library_space(
    ...
    if not _db_enabled(cfg):
        ...
        now = _utc_now()
        for doc in _MEMORY_DOCS.values():
            if doc.space_id == space_id and _memory_doc_visible(doc, user_id):
                doc.deleted_at = now
                doc.updated_at = now
        del _MEMORY_SPACES[space_id]
        return True
...
def _get_doc_count(config: SystemConfig, space_id: str) -> int:
    if not _db_enabled(config):
        return _memory_doc_count(space_id)
...
def get_library_doc_by_id(
    ...
    if not _db_enabled(cfg):
        ...
        if not doc or not _memory_doc_visible(doc, user_id):
            return None
        return doc
...
def delete_library_doc(
    ...
    if not _db_enabled(cfg):
        ...
        doc.deleted_at = now
        doc.updated_at = now
        space = _MEMORY_SPACES.get(doc.space_id)
        if space:
            space.doc_count = _memory_doc_count(doc.space_id, user_id)
            space.updated_at = now
        return True
...
def get_space_doc_count(
    ...
    cfg = config or get_config()
    if not _db_enabled(cfg):
        return _memory_doc_count(space_id, user_id)
