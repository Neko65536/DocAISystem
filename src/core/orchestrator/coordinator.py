...
class WorkflowCoordinator:
    ...
    def _document_editing_flow(self, task_spec: TaskSpec, progress_callback=None) -> WorkflowResult:
        ...
        return WorkflowResult(
            ...
            message=result.message if getattr(result, "message", None) else "文档编辑完成",
            data=result,
    ...
    def _entity_extraction_flow(self, task_spec: TaskSpec, progress_callback=None) -> WorkflowResult:
        ...
        output_filename = (
            ...
            else output_filename_from_source(
                task_spec.source_files[0].name or task_spec.source_files[0].path,
                "json",
            )
        )
        ...
        if task_spec.template_file:
            template_ext = Path(task_spec.template_file.name or task_spec.template_file.path).suffix.lower().lstrip(".") or "xlsx"
            template_output = Path(self.config.output_dir) / output_filename_from_source(
                task_spec.template_file.name or task_spec.template_file.path,
                template_ext,
            )
            filled_template = self.executor.fill_template(
                data=extracted_data,
                template=task_spec.template_file,
                output_template=str(template_output),
            )
            if filled_template:
                extracted_data.metadata = extracted_data.metadata or {}
