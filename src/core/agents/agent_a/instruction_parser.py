...
SUPPORTED_ACTIONS = {
    ...
    "insert_generated_content": {
        ...
        "patterns": [
            r"(?:在|于)?(?:文末|末尾|结尾|最后)(?:添加|加入|插入|补充|追加|生成).{0,24}(?:总结|总结性内容|纪要|结论|建议|说明|解答|内容|段落)",
            r"(?:在|于)?.{0,20}(?:后面|后|下面|下方|末尾)(?:添加|加入|插入|补充|追加|生成).{0,24}(?:总结|总结性内容|纪要|结论|建议|说明|解答|内容|段落)",
            r"(?:添加|加入|插入|补充|追加|生成).{0,24}(?:总结|总结性内容|纪要|结论|建议|说明|解答|内容|段落)",
            r"(?:根据|结合).{0,20}(?:全文|文档|原文).{0,20}(?:生成|撰写).{0,20}(?:并|后).{0,6}(?:添加|插入|加入)",
            r"(?:在|于).{0,20}(?:标题|章节|小节).{0,10}(?:下|下面|后|后面).{0,12}(?:添加|加入|插入|补充|生成).{0,24}(?:内容|总结|纪要|结论|建议|说明|解答|段落|风险提示)?",
        ],
    },
...
_ACTION_TYPE_ALIASES = {
    ...
    "insertgeneratedcontent": "insert_generated_content",
    "insert_generated_content": "insert_generated_content",
    "append_generated_content": "insert_generated_content",
...
def parse_instruction_with_llm_fallback(instruction: str, llm_service=None) -> Dict[str, Any]:
    ...
    prioritized_rule_payload = _build_rule_payload(instruction, service)
    if prioritized_rule_payload is not None and "insert_generated_content" in _extract_action_types(prioritized_rule_payload):
        return prioritized_rule_payload
    ...
    if llm_available:
        for attempt in range(2):
            ...
            if _is_valid_action_plan_payload(parsed):
                ...
                rule_payload = prioritized_rule_payload or _build_rule_payload(instruction, service)
...
def _call_llm_for_action_plan_json(instruction: str, llm_service, retry: bool = False) -> str:
    ...
    user_prompt = (
        f"用户指令: {instruction}\n"
        f"输出 schema: {schema}\n"
        "可用 action_type: bold_heading, insert_page_number, unify_style, reorder_paragraphs, batch_format, extract_content, replace_text, insert_toc, auto_column_width, freeze_header_row, remove_blank_lines, set_font_family, set_font_color, set_font_size, set_paragraph_alignment, set_line_spacing, set_first_line_indent, set_highlight, insert_table, insert_footer_text, insert_generated_content, set_heading_numbering\n"
...
def _normalize_action_params(action_type: str, params: Dict[str, Any], instruction: str) -> Dict[str, Any]:
    ...
    if action_type == "insert_generated_content":
        generation_instruction = str(
            normalized.get("generation_instruction")
            or normalized.get("instruction")
            or normalized.get("prompt")
            or normalized.get("raw_instruction")
            or ""
        ).strip()
        if not generation_instruction:
            generation_instruction = _extract_generated_content_instruction(instruction)
        normalized["generation_instruction"] = generation_instruction
        normalized["raw_instruction"] = str(normalized.get("raw_instruction") or instruction)
        normalized["content_label"] = str(
            normalized.get("content_label")
            or normalized.get("label")
            or _extract_generated_content_label(instruction)
        ).strip()
        if "prefer_heading" not in normalized:
            normalized["prefer_heading"] = bool(
                re.search(r"(?:新增|添加|加入|补充).{0,4}(?:一节|一段|一个章节|章节|小节)", instruction)
            )
        return normalized
...
def _build_action_payload(action_type: str, text: str, fallback: bool = False, llm_service=None) -> Dict[str, Any]:
    ...
    if action_type == "insert_generated_content":
        ...
        position = _extract_generated_content_position(text, bool(section_title))
        return {
            "action_type": action_type,
            "target": {
                "scope": "document",
            ...
            "params": {
                "generation_instruction": _extract_generated_content_instruction(text),
                "content_label": _extract_generated_content_label(text),
                "prefer_heading": bool(re.search(r"(?:新增|添加|加入|补充).{0,4}(?:一节|一段|一个章节|章节|小节)", text)),
...
def _segment_matches_action(action_type: str, rule: Dict[str, Any], seg: str) -> bool:
    if action_type == "bold_heading":
        if not re.search(r"标题|加粗|粗体|heading|bold", seg, flags=re.IGNORECASE):
            return False
...
def _extract_generated_content_position(text: str, has_section_title: bool) -> str:
    lowered = (text or "").lower()
    if re.search(r"文末|末尾|结尾|最后", text):
        return "end"
    if re.search(r"标题下|标题下面|章节下|章节下面|小节下|小节下面", text):
        return "after_section_heading"
    if re.search(r"标题后|标题后面|章节后|章节后面|小节后|小节后面", text):
        return "after_section"
    if has_section_title:
        return "after_section"
    if "append" in lowered or "at the end" in lowered:
        return "end"
    return "end"


def _extract_generated_content_label(text: str) -> str:
    label_patterns = [
        "会议纪要",
        "纪要",
        "风险提示",
...
def _extract_generated_content_instruction(text: str) -> str:
    content = (text or "").strip()
    content = re.sub(r"^[请麻烦帮我把将]\s*", "", content)
    content = re.sub(r"(输出|保存)(成|为).*$", "", content).strip()
    return content or "根据原文内容和用户要求生成需要插入文档的新内容。"
