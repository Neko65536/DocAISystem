...
def _split_text_chunks(content: str, chunk_size: int = FULL_TEXT_CHUNK_SIZE) -> List[str]:
    text = content or ""
    ...
    for line in text.splitlines(keepends=True):
        if len(line) > chunk_size:
            ...
            for start in range(0, len(line), chunk_size):
                part = line[start:start + chunk_size].strip()
                if part:
                    chunks.append(part)
            continue
...
def _run_llm_prompt(service, prompt: str, temperature: float = 0.3) -> str:
...
def _chunk_concurrency(total_chunks: int) -> int:
    try:
        configured = int(os.getenv("WORKFLOW_CHUNK_CONCURRENCY", "3"))
    except Exception:
        configured = 3
    return max(1, min(configured, total_chunks, 8))


def _run_llm_prompts_concurrently(service, prompts: List[str], temperature: float = 0.3) -> List[str]:
    if not prompts:
        return []
    workers = _chunk_concurrency(len(prompts))
    if workers <= 1:
        return [_run_llm_prompt(service, prompt, temperature) for prompt in prompts]

    results: List[str] = [""] * len(prompts)
    with ThreadPoolExecutor(max_workers=workers) as pool:
        future_map = {
            pool.submit(_run_llm_prompt, service, prompt, temperature): idx
...
def _strip_internal_chunk_prompt(text: str) -> str:
    if not text:
        return ""
    cleaned_lines: List[str] = []
    for line in str(text).splitlines():
        stripped = line.strip()
        if re.match(r"^以下是原始全文第\s*\d+\s*/\s*\d+\s*段[。．.]", stripped):
            continue
        if re.match(r"^当前内容为原始全文第\s*\d+\s*/\s*\d+\s*段[。．.]", stripped):
            continue
        if stripped.startswith("【内部处理说明】"):
            continue
        if "请只处理本段内容" in stripped and "不要因为这是分段" in stripped:
            continue
        cleaned_lines.append(line)
    return "\n".join(cleaned_lines).strip()


def _run_prompt_with_full_content(service, content: str, prompt: str, temperature: float = 0.3) -> str:
    if not content or len(content) <= FULL_TEXT_CHUNK_SIZE or content not in prompt:
        return _run_llm_prompt(service, prompt, temperature)
    ...
    for idx, chunk in enumerate(chunks, start=1):
        chunk_text = (
            f"【内部处理说明】当前内容为原始全文第{idx}/{total}段。只处理正文，不要输出本说明，不要输出分段编号，不要省略、总结或跳过正文。\n\n"
            f"{chunk}"
        )
        prompts.append(prompt.replace(content, chunk_text, 1))
    results = _run_llm_prompts_concurrently(service, prompts, temperature)
    return "\n\n".join(_strip_internal_chunk_prompt(str(result)) for result in results if result and _strip_internal_chunk_prompt(str(result)))


def _custom_or_default_prompt(custom_prompt: str, text: str, default_prompt: str) -> str:
    custom_prompt = str(custom_prompt or "").strip()
    if not custom_prompt:
        return default_prompt
    return custom_prompt.replace("{content}", text) if "{content}" in custom_prompt else f"{custom_prompt}\n{text}"


def _run_full_text_chunks(
    content: str,
    task_name: str,
    build_prompt,
    temperature: float = 0.3,
    joiner: str = "\n\n",
    aggregate_prompt_builder=None,
) -> str:
    ...
    try:
        if len(chunks) == 1:
            return _run_llm_prompt(service, build_prompt(chunks[0], 1, 1), temperature)

        prompts = [
            build_prompt(chunk, idx, len(chunks))
            for idx, chunk in enumerate(chunks, start=1)
        ]
        raw_results = _run_llm_prompts_concurrently(service, prompts, temperature)
        results = [_strip_internal_chunk_prompt(str(result)) for result in raw_results if result and _strip_internal_chunk_prompt(str(result))]

        combined = joiner.join(results)
        if aggregate_prompt_builder and combined.strip():
            return _run_llm_prompt(service, aggregate_prompt_builder(combined, len(chunks)), temperature)
...
def _chat_or_keep(content: str, prompt: str, task_name: str, temperature: float = 0.3) -> str:
    ...
    if not service:
        return content
    try:
        return _run_prompt_with_full_content(service, content, prompt, temperature)
    except Exception as e:
        logger.error(f"{task_name}失败: {e}")
        return content
...
def _translate_content(content: str, file_name: str, config: SystemConfig, config_values: Dict = None) -> Optional[str]:
    ...
    try:
        return _run_prompt_with_full_content(service, content, prompt, temperature=0.3)
    except Exception as e:
...
def _extract_summary_content(content: str, file_name: str, config_values: Dict) -> Optional[str]:
    ...
    try:
        return _run_prompt_with_full_content(service, content, prompt, temperature=0.5)
    except Exception as e:
...
def _enhance_text_content(content: str, file_name: str, config_values: Dict) -> Optional[str]:
    ...
    try:
        return _run_prompt_with_full_content(service, content, prompt, temperature=0.6)
    except Exception as e:
...
def _split_document_content(content: str, file_name: str, config_values: Dict) -> Optional[str]:
    ...
    custom_prompt = config_values.get("prompt", "").strip()
    if custom_prompt:
        prompt = custom_prompt.replace("{content}", content) if "{content}" in custom_prompt else f"{custom_prompt}\n{content}"
    else:
...
def _sensitive_masking_content(content: str, file_name: str, config_values: Dict) -> Optional[str]:
    ...
    try:
        return _run_prompt_with_full_content(service, content, prompt, temperature=0.2)
    except Exception as e:
