...
def _message_mode(metadata: Any) -> str:
    ...
    if isinstance(metadata, dict):
        return str(metadata.get("mode") or "").strip()
    return ""


def _user_visible_content(mode: str, content: Any) -> str:
    text = str(content or "")
    if mode == "document_editing":
        stripped = text.strip()
        if re.search(r"Agent[_ ]?A.*执行完成|agent_a.*执行完成", stripped, flags=re.IGNORECASE):
            return "文档编辑完成，已生成可下载文件"
        if "Agent_A最小骨架" in stripped or "最小骨架" in stripped:
            return "文档编辑任务已处理"
        if "缺少生成指令" in stripped:
            return "文档编辑失败：未能识别需要写入文档的新内容，请把要生成或追加的内容要求描述得更明确。"
        text = re.sub(r"\bAgent[_ ]?A\b", "文档编辑模块", text, flags=re.IGNORECASE)
        text = re.sub(r"\bagent_a\b", "文档编辑模块", text, flags=re.IGNORECASE)
    return text
...
def _resolve_file_reference(file_info: Dict[str, Any], cfg, session_id: str, kind: str) -> str:
    ...
    work_dir = Path(str(getattr(cfg, "work_dir", "workspace") or "workspace"))
    normalized_key = storage_key.replace("\\", "/")
    normalized_work_dir = work_dir.as_posix().strip("/")
    project_candidates = [project_root / storage_key]
    if normalized_work_dir and not normalized_key.startswith(f"{normalized_work_dir}/"):
        project_candidates.append(project_root / work_dir / storage_key)
...
def _ensure_files_in_db(files: List[Dict[str, Any]], session_id: str, cfg, user_id: Optional[str]) -> List[Dict[str, Any]]:
    ...
    for f in files:
        ...
        if storage_key in db_keys:
            ...
            update_file_selection(db_file.id, True, config=cfg, user_id=user_id)
            result.append({
                "id": db_file.id,
                "file_id": db_file.id,
            ...
            update_file_selection(session_file.id, True, config=cfg, user_id=user_id)
            result.append({
                "id": session_file.id,
...
def _message_to_dict(m) -> Dict[str, Any]:
    mode = _message_mode(m.metadata)
    content = _user_visible_content(mode, m.content) if m.role == "assistant" else m.content
    return {
        "id": m.id,
...
def _first_file_name(files: List[Dict[str, Any]], fallback: str = "output") -> str:
    for item in files or []:
        name = str(item.get("file_name") or item.get("name") or "").strip()
        if name:
            return Path(name).name
        path = str(item.get("file_path") or item.get("storage_key") or item.get("path") or "").strip()
        if path:
...
def _persist_generated_files(session_id: str, cfg, user_id: Optional[str], payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    candidate_paths = []
    for key in ("excel_path", "template_output", "output_json", "output_file"):
...
def _save_entity_extraction_files(
    session_id: str,
    cfg,
    user_id: Optional[str],
    entities_json: str,
    source_name: str = "entity_extraction",
) -> List[Dict[str, Any]]:
    ...
    # 1. 保存 JSON 文件
    json_name = output_filename_from_source(source_name, "json")
    ...
    # 2. 保存 XLSX 文件
    if entities:
        xlsx_name = output_filename_from_source(source_name, "xlsx")
...
def _save_table_filling_files(session_id: str, cfg, user_id: Optional[str], table_filling_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if output_json:
        ...
        if src_path.exists():
            source_name = Path(str(table_filling_data.get("excel_path") or src_path.name)).name
            json_name = output_filename_from_source(source_name, "json")
    ...
    if template_output:
        ...
        if src_path.exists():
            ...
            source_name = Path(str(template_source or src_path.name)).name
            xlsx_name = output_filename_from_source(source_name, ext)
...
def _recover_document_edit_generated_files(
    session_id: str,
    cfg,
    user_id: Optional[str],
    output_file: Optional[str],
) -> List[Dict[str, Any]]:
    """按输出路径回补文档编辑产生的输出文件记录。"""
    if not output_file:
        return []

    path_obj = Path(str(output_file))
    if not path_obj.exists():
        return []
    ...
    try:
        resolved_output = path_obj.resolve()
    except Exception:
        resolved_output = path_obj

    for row in rows:
        if getattr(row, "role", "") != "output":
            continue
        row_path = Path(str(getattr(row, "file_path", "") or ""))
        try:
            same_path = row_path.resolve() == resolved_output
        except Exception:
            same_path = str(row_path) == str(path_obj)
        same_name = str(getattr(row, "file_name", "") or "") == path_obj.name
        if same_path or same_name:
            row_file_name = getattr(row, "file_name", path_obj.name)
            row_file_path = getattr(row, "file_path", str(path_obj))
            file_ext = Path(str(row_file_path or row_file_name)).suffix.lower().lstrip(".") or "output"
            return [{
                "file_id": row.id,
    ...
    return _persist_generated_files(
        session_id,
        cfg,
        user_id,
...
async def send_message(session_id: str, request: SendMessageRequest, authorization: Optional[str] = Header(default=None)):
    ...
    requested_mode = (request.mode or session.current_mode or "default_conversation").strip()

    if request.files or request.template_files:
    ...
    effective_mode = agent_service.resolve_effective_mode(
        requested_mode,
        request.content,
        db_data_files,
        db_template_files,
    ...
    assistant_content = _user_visible_content(effective_mode, assistant_content)
    ...
    if not generated_files and effective_mode == "document_editing":
        task_spec = agent_service._build_task_spec(
            session_id,
            effective_mode,
            request.content,
            db_data_files,
        ...
        generated_files = _recover_document_edit_generated_files(
            session_id,
            cfg,
            current_user.id if current_user else None,
            task_spec.output_file,
        )
    if generated_files:
...
async def websocket_chat(websocket: WebSocket, session_id: str):
    ...
    try:
        ...
        while True:
            ...
            agent_service = AgentService()
            mode = agent_service.resolve_effective_mode(
                mode,
                user_content,
                files,
                template_files,
            ...
            if mode == "entity_extraction":
                ...
                extraction_files = _save_entity_extraction_files(
                    ...
                    current_user.id if current_user else None,
                    full_response,
                    _first_file_name(files, "entity_extraction"),
                )
            assistant_content = _user_visible_content(mode, assistant_content)
            ...
            if not generated_files and mode == "document_editing":
                task_spec = agent_service._build_task_spec(
                    session_id,
                    mode,
                    user_content,
