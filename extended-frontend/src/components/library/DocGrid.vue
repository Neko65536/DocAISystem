...
async function handleBatchDownload() {
  if (libraryStore.selectedCount === 0 || isBatchDownloading.value) return
  isBatchDownloading.value = true
  batchDownloadError.value = ''
  try {
    const ids = Array.from(libraryStore.selectedDocIds || [])
    const url = libraryApi.getBatchDownloadUrl(ids)
    const a = document.createElement('a')
    a.href = url
    a.download = 'library_docs.zip'
    document.body.appendChild(a)
    a.click()
    document.body.removeChild(a)
    setTimeout(() => {
    ...
    batchDownloadError.value = e.message || '批量下载失败，请重试'
    isBatchDownloading.value = false
  }
}

function handleSelectAllFiltered() {
  const ids = new Set((libraryStore.filteredDocs || []).map(doc => doc.id))
  libraryStore.selectedDocIds = ids
}
...
<template>
  ...
  >
    ...
    <div class="content-header">
      ...
      <div class="header-right">
        <div class="selected-info" v-if="libraryStore.selectedCount > 0">
          ...
          <button class="lib-btn" :disabled="isBatchDownloading" @click="handleBatchDownload">
            <span v-if="isBatchDownloading" class="btn-spinner"></span>
        ...
        <div v-else-if="libraryStore.currentSpaceId" class="header-actions">
          <button v-if="libraryStore.filteredDocs.length > 0" class="lib-btn" @click="handleSelectAllFiltered">
            <span>全选</span>
          </button>
    ...
    <div v-if="batchDownloadError" class="batch-download-error">
      {{ batchDownloadError }}
    </div>
...
.doc-card.selected {
  ...
  box-shadow: inset 0 0 0 1px rgba(56, 189, 248, 0.28), var(--shadow-md);
}
...
.doc-card:focus-within .doc-delete-btn {
  opacity: 1;
  pointer-events: auto;
}
