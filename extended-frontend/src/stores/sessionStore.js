...
function normalizeChatMode(mode) {
  return ALLOWED_CHAT_MODES.has(mode) ? mode : 'default_conversation'
}
...
function normalizeGeneratedFiles(rawFiles) {
  const normalizeOne = (file) => {
    if (!file) return null
    if (typeof file === 'string') {
      const path = file.trim()
      if (!path) return null
      return { file_path: path, file_name: path.split(/[\\/]/).pop() || 'result' }
    }
    if (typeof file === 'object') {
      const filePath = String(file.file_path || file.path || file.storage_key || '').trim()
      const hasFileId = file.file_id != null || file.id != null
      if (!hasFileId && !filePath) return null
      return {
        ...file,
        file_id: file.file_id ?? file.id,
        file_path: filePath || file.file_path,
        file_name: file.file_name || file.name || (filePath ? filePath.split(/[\\/]/).pop() : 'result'),
      }
    }
    return null
  }
  if (Array.isArray(rawFiles)) return rawFiles.map(normalizeOne).filter(Boolean)
  if (!rawFiles) return []
  const normalized = normalizeOne(rawFiles)
...
function normalizeMessageForResultDisplay(msg) {
  ...
  if (normalized.role === 'assistant' && metadata.mode === 'document_editing') {
    const content = String(normalized.content || '')
    if (/Agent[_ ]?A.*执行完成/i.test(content) || /agent_a.*执行完成/i.test(content)) {
      normalized.content = '文档编辑完成，已生成可下载文件'
    } else if (content.includes('Agent_A最小骨架') || content.includes('最小骨架')) {
      normalized.content = '文档编辑任务已处理'
    } else if (content.includes('缺少生成指令')) {
      normalized.content = '文档编辑失败：未能识别需要写入文档的新内容，请把要生成或追加的内容要求描述得更明确。'
    } else {
      normalized.content = content
        .replace(/\bAgent[_ ]?A\b/gi, '文档编辑模块')
        .replace(/\bagent_a\b/gi, '文档编辑模块')
    }
  }
...
export const useSessionStore = defineStore('session', () => {
  ...
  // 清除所有选中的文件，并同步服务端选中状态，避免刷新后恢复选中
  async function clearAllSelectedFiles() {
    const fileStore = useFileStore()
    const selections = []
    ;[...fileStore.tempFiles.data, ...fileStore.tempFiles.template].forEach(f => {
      const fileId = Number(f.id)
      if (Number.isInteger(fileId)) {
        selections.push({ file_id: fileId, is_selected: false })
      }
    })
    fileStore.tempFiles.data.forEach(f => { f.is_selected = false })
    fileStore.tempFiles.template.forEach(f => { f.is_selected = false })
    if (!currentSessionId.value || selections.length === 0) return
    try {
      await fileApi.updateSelection(currentSessionId.value, selections)
    } catch (e) {
      console.warn('[clearAllSelectedFiles] 同步文件选中状态失败:', e.message)
    }
  }

  /** 同步当前会话附件列表，供刷新和切换会话后恢复面板状态。 */
  async function loadFiles(sessionId, { preservePending = false } = {}) {
    if (!sessionId) return
    const fileStore = useFileStore()
    try {
      const res = await fileApi.list(sessionId)
      fileStore.setSessionFiles(res?.data_files || [], res?.template_files || [], { preservePending })
      if ((res?.data_files?.length || 0) + (res?.template_files?.length || 0) > 0) {
        fileStore.filesPanelCollapsed = false
      }
    } catch (e) {
      console.warn('[loadFiles] 加载会话文件失败:', e.message)
    }
  }

  async function toggleSessionFileSelection(file, type, isSelected) {
    const fileStore = useFileStore()
    fileStore.toggleFileSelection(file.id, type, isSelected)
    const fileId = Number(file.id)
    if (!currentSessionId.value || !Number.isInteger(fileId)) return
    try {
      await fileApi.updateSelection(currentSessionId.value, [
        { file_id: fileId, is_selected: isSelected },
      ])
    } catch (e) {
      console.warn('[toggleSessionFileSelection] 同步文件选中状态失败:', e.message)
    }
  }

  async function removeSessionFile(file, type) {
    const fileStore = useFileStore()
    fileStore.removeFile(file.id, type)
    const fileId = Number(file.id)
    if (!currentSessionId.value || !Number.isInteger(fileId)) return
    try {
      await fileApi.detach(currentSessionId.value, fileId)
    } catch (e) {
      console.warn('[removeSessionFile] 删除会话文件失败:', e.message)
      loadFiles(currentSessionId.value).catch(console.error)
    }
  }
  ...
  async function switchMode(mode) {
    ...
    if (currentSessionId.value) {
      ...
      if (idx !== -1) {
        sessions.value[idx] = { ...sessions.value[idx], current_mode: normalizeChatMode(mode) }
        saveSessionsCache(sessions.value, currentSessionId.value)
      }
      sessionApi.update(currentSessionId.value, { current_mode: normalizeChatMode(mode) }).catch(e =>
  ...
  async function loadSessions() {
    try {
      ...
      sessions.value = (res.items || []).map(s => ({
        ...s,
        current_mode: normalizeChatMode(s.current_mode),
      ...
      if (currentSessionId.value) {
        ...
        if (sess?.current_mode) {
          currentMode.value = normalizeChatMode(sess.current_mode)
  ...
  async function createSession() {
    ...
    try {
      ...
      // 加载新会话的文件列表（为空）
      loadFiles(res.session_id, { preservePending: true }).catch(console.error)
  ...
  async function selectSession(sessionId) {
    ...
    if (cachedMsgs && cachedMsgs.messages?.length > 0) {
      messages.value = cachedMsgs.messages.map(normalizeMessageForResultDisplay)
    } else {
  ...
  async function sendMessage(content, mode = 'default_conversation') {
    ...
    const effectiveMode = normalizeChatMode(mode || currentMode.value || 'default_conversation')
  ...
  async function init() {
    ...
    if (cached?.sessions?.length > 0) {
      sessions.value = cached.sessions.map(sess => ({
        ...sess,
        current_mode: normalizeChatMode(sess.current_mode),
      }))
