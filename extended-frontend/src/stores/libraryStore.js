...
export const useLibraryStore = defineStore('library', () => {
  ...
  const selectedDocs = computed(() => currentDocs.value.filter(d => selectedDocIds.value.has(d.id)))
  ...
  async function downloadSelectedDocs() {
    const ids = Array.from(selectedDocIds.value)
    if (ids.length === 0) return
    const blob = await libraryApi.downloadDocsBatch(ids)
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = ids.length === 1 ? `${selectedDocs.value[0]?.name || 'document'}.zip` : 'library_docs.zip'
